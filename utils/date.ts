...
/**
 * The function `findDateDifference` calculates the difference between two given dates in a specified
 * unit (e.g., days, weeks, months, etc.).
 * @param {Date} date1 - The `date1` parameter is the first date for which you want to calculate the
 * difference. //usually the current date
 ...
 * @param {keyof typeof dateUnits} diffUnit - The `diffUnit` parameter specifies the unit in which the
 * difference should be calculated. It can be any valid unit from the `dateUnits` enum (e.g., "days",
 * "weeks", "months"). Defaults to "days".
 * @returns The `findDateDifference` function returns the difference between two dates (`date1` and
 * `date2`) in the specified unit.
 */
export const findDateDifference = (
    date1: Date,
    date2: Date,
    diffUnit: keyof typeof dateUnits = 'days'
): number => {
    const diffInMilliseconds = Math.abs(date1.getTime() - date2.getTime());

    switch (dateUnits[diffUnit]) {
        case 'day':
            return Math.round(diffInMilliseconds / (1000 * 60 * 60 * 24));
        case 'week':
            return Math.round(diffInMilliseconds / (1000 * 60 * 60 * 24 * 7));
        case 'month':
            return Math.round(diffInMilliseconds / (1000 * 60 * 60 * 24 * 30.44)); // Approximate month length
        case 'year':
            return Math.round(diffInMilliseconds / (1000 * 60 * 60 * 24 * 365.25)); // Approximate year length
        case 'hour':
        ...
        default:
            throw new Error(`Unsupported diffUnit: ${diffUnit}`);
    }
};
 ...
 * @param {number} [expirationLeeway=0] - Optional leeway in milliseconds to account for expiration buffer.
 * @returns {boolean} - Returns true if the expiresAt is expired, false otherwise.
 */
export const isExpired = (expiresAt: string, expirationLeeway: number = 0): boolean => {
    ...
    const timeDifference = parsedTime - currentTime;

    if (!!expirationLeeway && expirationLeeway > 0 && timeDifference > 0 && timeDifference < expirationLeeway) {
        return false; // Not expired if within the leeway range
    }
